...
pub fn produce_packet(pkt: &Packet) -> Result<PacketInfo, Error> {
    println!("Packet Len: {:?}", pkt.len());
//    let pi: PacketInfo;
    match PacketHeaders::from_ethernet_slice(pkt) {
        Err(value) => Err(Error::from(format!("The packet can not parse to TCP packet.\n{:?}", value))),
        Ok(value) => {
...
//            println!("payload: {:?}", value.payload);
            ...
            Err(Error::from(format!("The packet can not parse to IP packet.\n{:?}", pkt)))
        }
    }
}
